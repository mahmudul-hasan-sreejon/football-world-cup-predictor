...
export default function Predictor() {
  ...
  useEffect(() => {
    try {
      setTheme(localStorage.getItem('wc26-theme') === 'light' ? 'light' : 'dark');
    } catch (e) {}
  }, []);
  ...
  function applyState(
    ...
    const cleaned = validatePicks({ order: nextOrder, thirds: nextThirds, picks: nextPicks });
    setOrder(nextOrder);
  ...
  function autoPick() {
    ...
    const thirdTeams = GROUPS.map((g) => ({ g, r: BYID[nextOrder[g][2]].rating })).sort(
      (a, b) => b.r - a.r,
    );
  ...
  function copySummary() {
    ...
    GROUPS.forEach((g) => {
      ...
      s +=
        `Group ${g}: ` +
        (o.length ? o.map((id, i) => `${i + 1}.${BYID[id].name}`).join('  ') : '—') +
        '\n';
    });
    s +=
      '\nBEST 3RD-PLACE QUALIFIERS\n' +
      (thirds.size === 8
        ? [...thirds].sort().map((g) => BYID[order[g][2]].name).join(', ')
        : '(not chosen)') +
      '\n';
    s += '\nKNOCKOUT\n';
    ([
  ...
  function toggleTheme(e: MouseEvent<HTMLButtonElement>) {
    ...
    if (doc.startViewTransition && !reduceMotion) {
      ...
      doc.startViewTransition(() => commitTheme(next)).ready.then(() => {
      ...
      <TabsContent value="groups" className="stage show">
        <div className="stage-head">
          <div>
            <h2>Group Stage</h2>
            <p>
              Tap teams in your predicted finishing order — 1st, 2nd, 3rd. The last spot fills in
              automatically. Top two of each group advance; third place may still qualify.
            </p>
          </div>
          <div className="pill">
          ...
          {GROUPS.map((g) => {
            ...
            return (
              <div className="gcard" key={g}>
                ...
                <div className="ghint">{o.length >= 3 ? '✓ Ranked' : 'Tap teams in finishing order'}</div>
              </div>
            );
          })}
        </div>
      ...
      <TabsContent value="thirds" className="stage show">
        <div className="stage-head">
          <div>
            ...
            <p>
              Eight of the twelve group-stage third-placed teams advance to the Round of 32. Pick
              the <b>8</b> you think make it. Which groups they come from determines the exact
              knockout seeding.
            </p>
          </div>
          <div className="pill">
      ...
      <TabsContent value="ko" className="stage show">
        <div className="stage-head">
          <div>
            ...
            <p>
              Single elimination, Round of 32 through the Final. Tap the team you predict to win
              each tie — winners advance automatically. Change an earlier pick and the rounds after
              it reset.
            </p>
          </div>
          <div className="pill">
                      ...
                      <div className="matches">
                        {r.ms.map((m) => {
                          const isFinal = m.id === 104,
                            isThird = m.id === 103;
                          const tag = isFinal ? '★ FINAL' : isThird ? '3rd-place' : 'Match ' + m.id;
                          return (
                            <div
